...
void* threadfunc(void* thread_param)
{
    ...
    if (rc != 0) {thread_func_args->thread_complete_success = false; return thread_func_args;}
...
bool start_thread_obtaining_mutex(pthread_t *thread, pthread_mutex_t *mutex,int wait_to_obtain_ms, int wait_to_release_ms)
{
    ...
    int rc = pthread_create(thread, NULL, threadfunc, thr_data);
    if (rc != 0) { free(thr_data); return false;}
